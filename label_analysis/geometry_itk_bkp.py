...
class LabelMapGeometryITK(LabelMapGeometry):
    ...
    def labels(self):
        if self.nbrhoods.empty:
            return []
        return self.nbrhoods.label_cc.tolist()
...
#SECTION:-------------------- dust--------------------------------------------------------------------------------------
    ...
    size_metric = "major_axis" if L.compute_feret == False else "feret"
    inds_small = L.nbrhoods[size_metric] < dusting_threshold
    if remove_flat == True:
        inds_flat = L.nbrhoods["flatness"] == 0
        inds_small = inds_small | inds_flat
    inds_small_final = L.nbrhoods.loc[inds_small].index
    L.remove_labelobjects(inds_small_final)
# %%
# %%
#SECTION:--------------------  remove_labelobjects--------------------------------------------------------------------------------------
    df_inds = inds_small_final
